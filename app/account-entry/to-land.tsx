...
function Row({ item, onPress }: { item: Transaction; onPress: () => void }) {
  return (
    <TouchableOpacity style={styles.row} onPress={onPress}>
      ...
      <Text style={styles.amount}>{formatCurrency(item.cash_out || 0)}</Text>
    </TouchableOpacity>
  );
}

export default function ToLandScreen() {
  const router = useRouter();
  const { transactions } = useTransactions();
  const rows = useMemo(
    () => [...transactions]
      .filter((t) => t.nagar_name === ACCOUNTS_LEDGER && t.category === 'Govt' && (t.cash_out || 0) > 0)
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
    [transactions]
  );
  const total = rows.reduce((sum, r) => sum + (r.cash_out || 0), 0);

  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.back()}><Text style={styles.back}>← Back</Text></TouchableOpacity>
        <Text style={styles.title}>To Land Entries</Text>
        <View style={{ width: 56 }} />
      </View>
      <View style={styles.totalBox}>
        <Text style={styles.totalLabel}>Total To Land</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No To Land entries.</Text>}
...
const styles = StyleSheet.create({
  ...
  totalValue: { marginTop: 4, fontSize: 22, fontWeight: '900', color: AppColors.expense },
  ...
  amount: { fontSize: 14, fontWeight: '800', color: AppColors.expense },
